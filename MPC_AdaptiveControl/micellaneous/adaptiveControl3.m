function [c, Ap, Bp, Cp] = adaptiveControl3(Q_bar, Y, Ck, p, params, bnds)
n = params.n;
m = params.m;
Ts = params.Ts;

assert(p/length(bnds) == round(p/length(bnds)), 'bounds need to repeat: p must be divisible by 2')
...
[Ac, Ax, b1] = make_constraints(Ap, Bp, Cp, p, bnds);
...
% Cp2 = Cp_inter(1, 1).*[last_row_A(1, 1:m)];
% Cp2 = [Cp2; Cp_inter(1, m+1).*last_row_A(1, 1:m)];
% 
% Cp = Cp + Cp2;

% for i = 1: m/2
%     
%     Cp(1, (i-1)*2 + 1 ) = Cp_inter(1, i) + last_row_A(1, i)*P(m+1);
%     Cp(2,  2*i ) = Cp_inter(1, m+i) + last_row_A(1, i)*P(2*m+1);
%     
% end
...
function [Ac, Ax, b] = make_constraints(PhiP, Bp, C, p, bnds)
% Make psi
