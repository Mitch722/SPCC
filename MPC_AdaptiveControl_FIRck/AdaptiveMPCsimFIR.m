...
for k = 1 : Time_out/TsFast
    ...
    if k == adaptTime - 1
        % Try statement 
        [~, ~, RstarModel, entry] = ACSA_FIR(M_samps, y(:, 1:k-1), Ck(:, 1:k-1), p, params, bnds, A-B*K_opt, B, C, K_opt);      
    ...
    if k > adaptTime  && k / ratioTs == round(k/ratioTs)
        ...
        if k/25 == round(k/25)
            ...
            [~, ~, RstarModel, entry, no_coefs] = ACSA_FIR(M_samps, y(:, 1:k-1), Ck(:, 1:k-1), p, params, bnds, A-B*K_opt, B, C, K_opt);
...
%         Linv1 = inv(L21);
        ...
        ck = mpcqpsolver(Linv, f, Dc2, b, [], zeros(0,1), false(size(b)), options);
        if isempty(ck) || abs(ck(1)) > 10
            c = 0;
        else
            c = ck(1);
